...
def test_vendored_vertex_cli_version_matches_python_package() -> None:
    ...
    assert cli_package["version"] == "1.2.6"
    assert 'version("1.2.6 (Vertex)"' in cli_bundle
    assert 'console.log(`${"1.2.6"} (Vertex)`)' in cli_bundle
    assert 'vertex ${RESET}${rgb(...ACCENT)}v${"1.2.6"}' in cli_bundle
